...
- **Multi-modal support** — anat, func, dwi, fmap, perf, PET, and more
...
- **Safe deletion** — deletes source DICOMs only after verifying successful NIfTI output
- **Dry-run mode** — preview every action without modifying anything
- **Parallel conversion** — run N conversions simultaneously
- **JSON enrichment** — automatically injects `TaskName`, `RepetitionTime`, and `IntendedFor` placeholders
...
Patterns use bash globbing and are matched case-insensitively.
...
5. **Set IntendedFor** — add `IntendedFor` paths in fieldmap JSON sidecars
